...
export const getResolvers = (subgraph: Subgraph): Record<string, any> => {
  ...
  return {
    Query: {
      ScalarTesting: async (_: any, args: any, ctx: any) => {
        return {
    ...
    Mutation: {
      ScalarTesting_createDocument: async (_: any, args: any) => {
        const driveId: string = args.driveId || DEFAULT_DRIVE_ID;
      ...
      ScalarTesting_addBoolean: async (_: any, args: any) => {
        const driveId: string = args.driveId || DEFAULT_DRIVE_ID;
        const docId: string = args.docId || "";
        const doc = await reactor.getDocument(driveId, docId);
      ...
      ScalarTesting_addFileBase64: async (_: any, args: any) => {
        const driveId: string = args.driveId || DEFAULT_DRIVE_ID;
        const docId: string = args.docId || "";
        const doc = await reactor.getDocument(driveId, docId);
        ...
        await reactor.addAction(
          driveId,
          docId,
          actions.addFileBase64({ ...args.input }),
        );

        return (doc.header.revision["global"] ?? 0) + 1;
      },

      ScalarTesting_removeFileBase64: async (_: any, args: any) => {
        const driveId: string = args.driveId || DEFAULT_DRIVE_ID;
        const docId: string = args.docId || "";
        const doc = await reactor.getDocument(driveId, docId);
        ...
        await reactor.addAction(
          driveId,
          docId,
          actions.removeFileBase64({ ...args.input }),
        );
